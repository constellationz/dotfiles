...
-- To make sure that the "randomly" generated colors look cohesive, only the first color is truly random, the rest
-- are generated by offseting the hue by +33 degrees
...
local function create_button_image(name, is_focused, event, is_on)
    ...
    if not _private[key_color] then
        ...
        -- Then the color is lightened if the button is being hovered over, or darkened if it is being pressed,
...
function _private.add_window_decorations(c)
    ...
    local background_fill = shapes.color(client_color)
    ...
    local corner_top_left_img = create_corner_top_left({
        ...
        stroke_source = shapes.color(stroke_color),
    })
